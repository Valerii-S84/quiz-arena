            ...
            try:
                ...
                message = await bot.send_photo(
                    ...
                    photo=BufferedInputFile(
                        card_png,
                        filename=f"daily_cup_{tournament_id}_{user_id}.png",
                    ),
                    caption=caption,
                )
                sent += 1
...
def enqueue_daily_cup_proof_cards(*, tournament_id: str) -> None:
    try:
        if _is_celery_task(run_daily_cup_proof_cards):
            run_daily_cup_proof_cards.delay(tournament_id=tournament_id)
        else:
            run_async_job(run_daily_cup_proof_cards_async(tournament_id=tournament_id))
    except Exception as exc:
        logger.warning(
...
def run_daily_cup_proof_cards(*, tournament_id: str) -> dict[str, int]:
    return run_async_job(run_daily_cup_proof_cards_async(tournament_id=tournament_id))
