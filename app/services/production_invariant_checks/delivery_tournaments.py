...
_EXPECTED_DELIVERY_ZERO_OUTCOMES_SQL = f"""
    SELECT count(*)
    FROM tournaments t
    WHERE t.type = 'PRIVATE'
      AND {_RECENT_PHASE_SQL}
      AND EXISTS (
        SELECT 1
...
def build_tournament_delivery_checks(recent_cutoff: datetime) -> list[InvariantCheck]:
    return [
        ...
        build_check(
            ...
            params={"recent_cutoff": recent_cutoff},
            description="Private tournament participant is missing a current-phase attempt.",
        ),
    ]
