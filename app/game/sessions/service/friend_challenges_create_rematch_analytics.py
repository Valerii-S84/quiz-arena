...
def _rematch_series_id_value(*, rematch: FriendChallenge) -> str | None:
    if rematch.series_id is None:
        return None
    return str(rematch.series_id)
...
async def emit_rematch_duel_created_events(
    ...
    *,
    ...
    await emit_analytics_event(
        ...
        happened_at=happened_at,
        user_id=initiator_user_id,
        payload=_duel_revanche_payload(
            rematch=rematch,
            source_challenge_id=source_challenge_id,
            opponent_user_id=opponent_user_id,
        ),
