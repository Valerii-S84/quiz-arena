...
async def handle_answer(callback: CallbackQuery) -> None:
    await answer_flow.handle_answer(callback, context=_build_answer_flow_context())


def _build_answer_flow_context() -> answer_context.AnswerFlowContext:
    return answer_context.AnswerFlowContext(
        parse_answer_callback=gameplay_callbacks.parse_answer_callback,
        services=answer_context.AnswerFlowServices(
            session_local=SessionLocal,
            user_onboarding_service=UserOnboardingService,
        ...
        analytics=answer_context.AnswerFlowAnalytics(
            emit_event=emit_analytics_event,
            event_source_bot=EVENT_SOURCE_BOT,
        ),
        branches=answer_context.AnswerFlowBranches(
        ...
        rendering=answer_context.AnswerFlowRendering(
            build_question_text=_build_question_text,
