...
from app.bot.handlers.gameplay_flows import daily_cup_flow, daily_cup_menu_flow
...
def register(router: Router) -> None:
    ...
    router.callback_query(F.data.regexp(gameplay_callbacks.DAILY_CUP_PROOF_RE))(
        handle_daily_cup_proof
    )
...
async def handle_daily_cup_menu(callback: CallbackQuery) -> None:
    gameplay = _gameplay()
    try:
...
async def handle_daily_cup_proof(callback: CallbackQuery) -> None:
    if callback.data is None:
        await callback.answer(TEXTS_DE["msg.system.error"], show_alert=True)
        return
    tournament_id = gameplay_callbacks.parse_uuid_callback(
        pattern=gameplay_callbacks.DAILY_CUP_PROOF_RE,
        callback_data=callback.data,
    )
    if tournament_id is None:
        await callback.answer(TEXTS_DE["msg.system.error"], show_alert=True)
