...
| P8-CODEX-FINAL-P2-FIX | 8 | Resolve final Codex P2 payment review findings before merge | DONE | this final safety commit | `.venv/bin/python -m pytest --capture=no -q tests/api/test_telegram_webhook.py tests/bot/test_payments_handler.py tests/bot/test_payments_handler_flow.py tests/bot/test_payments_handler_flow_offer.py tests/economy/test_purchase_credit_service.py tests/economy/test_purchase_refund_state_units.py tests/economy/test_purchase_refund_errors_units.py tests/economy/test_purchase_refund_branch_units.py tests/economy/test_purchase_refund_debit_units.py tests/economy/test_purchase_refund_additional_units.py` -> `67 passed in 17.33s`; `.venv/bin/python -m pytest --capture=no -q tests/services/test_payment_reconciliation.py tests/services/test_telegram_stars.py tests/workers/test_telegram_stars_reconciliation_task.py tests/workers/test_payments_reliability_async.py tests/workers/test_payments_reliability_async_credit_batch.py tests/workers/test_payments_reliability_async_credit_single.py tests/workers/test_payments_reliability_task.py` -> `55 passed in 25.14s`; `.venv/bin/python -m pytest --capture=no -q tests/api/test_internal_promo_refunds_unit.py tests/integration/test_purchase_refund_integration.py tests/integration/test_economy_invariants_b_refund_symmetry_integration.py tests/integration/test_purchase_premium_integration.py tests/integration/test_payments_idempotency_purchase_flow_integration.py tests/integration/test_payments_idempotency_recovery_integration.py tests/integration/test_payments_idempotency_reconciliation_integration.py tests/integration/test_payments_idempotency_promo_rollback_integration.py` -> `21 passed in 44.85s`; `.venv/bin/ruff check app tests scripts` -> `All checks passed!`; `.venv/bin/black --check app tests scripts` -> `1341 files would be left unchanged`; `.venv/bin/isort --check-only app tests scripts` -> pass; `.venv/bin/mypy app tests` -> `Success: no issues found in 1326 source files`; `git diff --check` -> pass | `PASS_WITH_NOTES` | Raw Telegram charge IDs are no longer emitted in touched payment/refund/update logs; safe `telegram_payment_charge_id_hash` values remain. Generic refunds now reject `FAILED_CREDIT_PENDING_REVIEW`; only the Telegram provider-confirmed refund update path passes explicit evidence. Normal `CREDITED` and `PAID_UNCREDITED` refunds remain covered. No migrations, deploy, production DB writes, `.env*`, secrets, or production config changes. |
...
### P8 - `7afc857` - `docs(payments): finalize reliability runbooks`
...
### P8-PR-FIX2 - `60d689d` - `fix(payments): address refund reconciliation review`

Auditor verdict: `PASS_WITH_NOTES`

Auditor notes:

- No blocking issues found in local follow-up verification after addressing the second post-PR P2
  review batch.
- Paid purchase constraint preflights now include `FAILED_CREDIT_PENDING_REVIEW` for missing
  `telegram_payment_charge_id` and missing `paid_at`, while P7B `credited_at` semantics remain
  unchanged.
- Stars reconciliation exact recovery classification now requires matching `invoice_payload`; a
  same-user/same-amount candidate with a different invoice payload is ambiguous, not exact.
- Stars reconciliation backlog scans are bounded to five Telegram pages per run and expose
  `transactions_backlog_truncated` when the cap is reached.
- `message.refunded_payment` updates now reach the payments router, look up the purchase by
  Telegram charge id, validate user/invoice, and call the existing idempotent
  `PurchaseService.refund_purchase` path. Refund failures are re-raised so Telegram update
  processing enters the existing Celery retry/final-failure path instead of being silently marked
  processed.
- The session-start energy idempotency regression test still proves the post-lock ledger replay
  check is not skipped.
- A raw local `scripts/payment_reliability_checks.py --json` invocation failed before DB access due
  missing local app settings; the accepted checker run used explicit test-only dummy settings and
  `quiz_arena_test`, and returned all checks `OK`.
- No migrations, deploy config, protected paths, `.env*`, secrets, production deploy, or production
  DB writes were introduced.
- Safe defaults remain unchanged:
  `TELEGRAM_STARS_RECONCILIATION_ENABLED=false`,
  `TELEGRAM_STARS_RECONCILIATION_DRY_RUN=true`,
  `TELEGRAM_STARS_AUTO_RECOVERY_ENABLED=false`.
- Non-blocking note: the bounded Stars scan still has no persisted cursor; hitting
  `transactions_backlog_truncated=true` is an operator signal that deeper history was not scanned
  in that run.

Lead response: Accepted locally; push the branch for external re-review, keep the PR unmerged, and
do not deploy or mutate production data.

### P8-PR-FIX3 - `cfd6366` - `fix(payments): harden Stars refund reconciliation`

Auditor verdict: `PASS_WITH_NOTES`

Auditor notes:

- No blocking issues found in the full local PR re-audit after `cfd6366`.
- The prior provider-refund blocker is resolved: outgoing `invoice_payment` Star transactions now
  classify as already resolved when locally `REFUNDED`, or high-severity
  `PROVIDER_REFUND_REQUIRES_REVIEW` when a credited or otherwise unresolved local purchase needs
  manual review.
- The refund webhook race blocker is resolved: `message.refunded_payment` now falls back from
  charge-id lookup to invoice-payload lookup under lock, validates user/payload/charge conflict,
  records safe payment evidence, and then uses the existing idempotent `refund_purchase` path.
- The oldest-history Stars scan blocker is resolved: reconciliation estimates total history and
  reads the recent tail window instead of only the first chronological pages.
- The auto-recovery loop blocker is resolved: per-transaction classification/recovery exceptions are
  logged with sanitized hashes, counted, and do not abort later review/recovery decisions.
- The manual-review retention blocker is resolved: age-based outbox cleanup excludes `OPEN` rows;
  unit and integration tests cover the predicate and old open review preservation.
- The manual-review actionability blocker is resolved: review payloads include amount, date,
  Telegram user clue, type/direction, and candidate ids/count while still excluding raw Telegram
  payload, raw invoice payload, raw charge id, bot token, and webhook secret.
- Non-blocking notes: provider refund reconciliation remains manual-review-first rather than
  automatic de-crediting; Stars `limit=1` count probes are not included in `pages_fetched`; review
  payload stores raw `telegram_user_id`; review dedupe remains best-effort until a dedicated table
  or DB constraint is approved.
- Lead-side verification is complete: targeted payment/reconciliation/refund/session suites passed,
  broad `ruff`, `black --check`, `isort --check-only`, and `mypy app tests` passed, raw test DB
  payment reliability checker returned all 12 checks `OK`, and webhook default
  `allowed_updates=[]` returned `OK`.
- Local GitHub PR metadata check returned `mergeable=true` and `merged=false`.
- No migrations, deploy config, protected paths, `.env*`, secrets, production deploy, or production
  DB writes were introduced.
- Safe defaults remain unchanged:
  `TELEGRAM_STARS_RECONCILIATION_ENABLED=false`,
