...
from tests.game.daily_arena_golden_extended_support import arena_tournament, empty_daily_cup_result
...
async def test_daily_arena_messaging_returns_empty_for_invalid_tournament_id() -> None:
    ...
    assert result == empty_daily_cup_result()
...
@pytest.mark.parametrize(
    ...
    [
        (None, [make_standing_row(user_id=101, place=1)], empty_daily_cup_result()),
        (
            arena_tournament(status=TOURNAMENT_STATUS_REGISTRATION),
            [make_standing_row(user_id=101, place=1)],
            empty_daily_cup_result(),
        ),
        (
            arena_tournament(status=TOURNAMENT_STATUS_COMPLETED),
            [],
            empty_daily_cup_result(),
        ),
    ],
...
async def test_daily_arena_messaging_round_pipeline_fetches_round_matches_for_arena(
    ...
    # GOLDEN: фіксує поточну поведінку, не змінювати без рев'ю
    tournament = arena_tournament(status="ROUND_2", current_round=2)
    standings = [
