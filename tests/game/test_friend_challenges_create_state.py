...
from app.game.sessions.service import (
    friend_challenges_create_state,
...
@pytest.mark.asyncio
async def test_load_friend_challenge_rematch_context_delegates_to_followup_loader(
    ...
    monkeypatch.setattr(
        friend_challenges_create_state.friend_challenges_followup_state,
        "load_friend_challenge_followup_context",
        _fake_load_followup_context,
    )

    session = _Session()
    result = await friend_challenges_create_state.load_friend_challenge_rematch_context(
        session,
