...
from app.game.sessions.service import friend_challenges_manage_runtime
...
async def test_repost_friend_challenge_as_open_creates_repost_and_emits_events(
    ...
    monkeypatch.setattr(
        friend_challenges_manage_runtime,
    ...
    result = await friend_challenges_manage_runtime.repost_friend_challenge_as_open(
        _Session(),
        user_id=11,
    ...
    assert create_calls == [
        {
            ...
            "now_utc": NOW_UTC,
            "challenge_type": friend_challenges_manage_runtime.DUEL_TYPE_OPEN,
    ...
    assert analytics_events == [
        {
            ...
            "source": friend_challenges_manage_runtime.EVENT_SOURCE_BOT,
            "happened_at": NOW_UTC,
            "user_id": 11,
            "payload": {
...
async def test_repost_friend_challenge_as_open_delegates_state_loading(
    ...
    await friend_challenges_manage_runtime.repost_friend_challenge_as_open(
        _Session(),
        user_id=11,
        challenge_id=challenge.id,
        now_utc=NOW_UTC,
...
async def test_cancel_friend_challenge_by_creator_marks_canceled_and_returns_snapshot(
    ...
    result = await friend_challenges_manage_runtime.cancel_friend_challenge_by_creator(
        _Session(),
        user_id=11,
        challenge_id=challenge.id,
        now_utc=NOW_UTC,
...
async def test_cancel_friend_challenge_by_creator_delegates_state_loading(
    ...
    await friend_challenges_manage_runtime.cancel_friend_challenge_by_creator(
        _Session(),
        user_id=11,
        challenge_id=challenge.id,
        now_utc=NOW_UTC,
