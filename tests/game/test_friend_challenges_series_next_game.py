...
from app.game.sessions.service import friend_challenges_series_next_game
...
async def test_create_friend_challenge_series_next_game_rejects_without_series_metadata(
    ...
    monkeypatch.setattr(
        friend_challenges_series_next_game,
    ...
    with pytest.raises(FriendChallengeAccessError):
        await friend_challenges_series_next_game.create_friend_challenge_series_next_game(
            _Session(),
...
async def test_create_friend_challenge_series_next_game_creates_followup_duel_and_emits_events(
    ...
    result = await friend_challenges_series_next_game.create_friend_challenge_series_next_game(
        _Session(),
    ...
    assert series_event_calls == [
        {
            ...
            "opponent_user_id": 101,
            "happened_at": NOW_UTC,
            "source": friend_challenges_series_next_game.EVENT_SOURCE_BOT,
            "initiator_user_id": 202,
        }
    ]
