...
async def test_handle_buy_skips_duel_click_event_without_duel_context(monkeypatch) -> None:
    ...
    async def _unexpected_duel_click(*args, **kwargs):
        raise AssertionError("shop purchase must not emit duel paywall analytics")

    monkeypatch.setattr(payments.UserOnboardingService, "ensure_home_snapshot", _fake_home_snapshot)
    monkeypatch.setattr(payments.PurchaseService, "init_purchase", _fake_init_purchase)
    monkeypatch.setattr(payments.PurchaseService, "mark_invoice_sent", _fake_mark_invoice_sent)
    monkeypatch.setattr(payments, "_emit_duel_paywall_click", _unexpected_duel_click)
...
async def test_handle_buy_emits_duel_click_event_for_duel_context(monkeypatch) -> None:
    ...
    async def _fake_duel_click(session, *, user_id, product_code, happened_at):
        duel_clicks.append({"user_id": user_id, "product_code": product_code})

    monkeypatch.setattr(payments.UserOnboardingService, "ensure_home_snapshot", _fake_home_snapshot)
    monkeypatch.setattr(payments.PurchaseService, "init_purchase", _fake_init_purchase)
    monkeypatch.setattr(payments.PurchaseService, "mark_invoice_sent", _fake_mark_invoice_sent)
    monkeypatch.setattr(payments, "_emit_duel_paywall_click", _fake_duel_click)
