...
from app.bot.handlers.gameplay_flows import daily_cup_flow, daily_cup_share_flow
...
@pytest.mark.asyncio
async def test_handle_daily_cup_share_result_repeat_tap_thanks_without_requeue(monkeypatch) -> None:
    async def _fake_home_snapshot(session, *, telegram_user):
        del session, telegram_user
        return SimpleNamespace(user_id=101)
    ...
    monkeypatch.setattr(daily_cup_proof_cards, "enqueue_daily_cup_proof_cards", _fake_enqueue)

    message = DummyMessage()
    message.reply_markup = build_daily_cup_share_keyboard(
    ...
    assert enqueued == []
    assert callback.answer_calls == [
        {"text": TEXTS_DE["msg.daily_cup.share.thanks"], "show_alert": False}
    ]


@pytest.mark.asyncio
async def test_handle_daily_cup_request_proof_card_rejects_stale_tournament(monkeypatch) -> None:
    stale_now_utc = datetime(2026, 3, 2, 10, 0, tzinfo=timezone.utc)

    class _FrozenDateTime(datetime):
        @classmethod
        def now(cls, tz=None):
            return stale_now_utc if tz is not None else stale_now_utc.replace(tzinfo=None)
    ...
    async def _fake_get_lobby(session, *, tournament_id, viewer_user_id):
        ...
        return SimpleNamespace(
            tournament=SimpleNamespace(
                tournament_id=UUID("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"),
                status="COMPLETED",
                registration_deadline=datetime(2026, 3, 1, 17, 0, tzinfo=timezone.utc),
            ),
            viewer_joined=True,
            participants=(SimpleNamespace(user_id=101, score=Decimal("3.0")),),
        )
    ...
    monkeypatch.setattr(daily_cup_share_flow, "datetime", _FrozenDateTime)
    monkeypatch.setattr(daily_cup_proof_cards, "enqueue_daily_cup_proof_cards", _fake_enqueue)
