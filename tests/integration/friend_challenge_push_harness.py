...
async def telegram_user_id(user_id: int) -> int:
...
async def submit_friend_round_answer(
    *,
    user_id: int,
    challenge_id,
    round_no: int,
...
async def run_friend_answer_branch(*, actor_user_id: int, actor_telegram_user_id: int, result):
    ...
    callback = DummyCallback(
        data="answer:test:0",
        from_user=SimpleNamespace(id=actor_telegram_user_id, first_name="Friend"),
...
async def create_joined_duel(now_utc: datetime):
