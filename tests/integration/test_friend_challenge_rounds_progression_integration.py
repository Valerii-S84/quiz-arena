...
async def _create_joined_challenge(
    session,
    *,
    creator_user_id: int,
...
async def _resolve_correct_option(session, *, game_session_id: UUID) -> int:
    game_session = await QuizSessionsRepo.get_by_id(session, game_session_id)
    assert game_session is not None

    question = await get_question_by_id(
        session,
        game_session.mode_code,
        question_id=game_session.question_id or "",
        local_date_berlin=game_session.local_date_berlin,
    )
...
async def test_friend_challenge_uses_same_question_for_both_users_and_updates_round_score() -> None:
    ...
    async with SessionLocal.begin() as session:
        challenge = await _create_joined_challenge(
            session,
            creator_user_id=creator_user_id,
    ...
    async with SessionLocal.begin() as session:
        correct_option = await _resolve_correct_option(session, game_session_id=creator_session_id)
...
async def test_friend_challenge_completes_and_sets_winner() -> None:
    ...
    async with SessionLocal.begin() as session:
        ...
        for round_no in range(1, 8):
            ...
            correct_option = await _resolve_correct_option(
                session,
                game_session_id=creator_round.start_result.session.session_id,
            )
...
async def test_friend_challenge_creator_can_continue_without_waiting_for_opponent() -> None:
    ...
    async with SessionLocal.begin() as session:
        ...
        correct_option = await _resolve_correct_option(
            session,
            game_session_id=round_one.start_result.session.session_id,
        )
        await GameSessionService.submit_answer(
            session,
            user_id=creator_user_id,
