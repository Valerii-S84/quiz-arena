...
def upgrade() -> None:
    ...
    op.create_index(
        ...
        ["user_id"],
        unique=True,
        postgresql_where=sa.text("entitlement_type = 'PREMIUM' AND status = 'ACTIVE'"),
    )
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.CheckConstraint(
            "source IN ('FREE','MEGA_PACK','PREMIUM')", name="ck_mode_access_source"
        ),
        sa.CheckConstraint(
            "status IN ('ACTIVE','EXPIRED','REVOKED')", name="ck_mode_access_status"
        ),
        sa.ForeignKeyConstraint(["source_purchase_id"], ["purchases.id"]),
        sa.ForeignKeyConstraint(["user_id"], ["users.id"]),
